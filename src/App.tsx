...
function MotionScene({ onOpen }: { onOpen?: () => void }) {
  return (
    <div className="motion-scene" onClick={onOpen} role={onOpen ? 'button' : undefined} tabIndex={onOpen ? 0 : undefined}>
...
function Editor({ panel, setPanel }: { panel: Panel; setPanel: (panel: Panel) => void }) {
  ...
  return (
    <div className="editor-shell">
      <header className="topbar">
        <button className="brand" onClick={() => setPanel('editor')}>ATOMIZER <span>Motion Lab</span></button>
        <nav className="nav-actions">
          <button onClick={() => setPlaying(value => !value)}>{playing ? 'Pause' : 'Play'}</button>
          <button onClick={() => setShowHelp(value => !value)}>Shortcuts</button>
          <button onClick={() => setPanel('wallpapers')}>Wallpaper Hub</button>
          <button onClick={() => setPanel('scraper')}>Scraper</button>
          <button onClick={() => setPanel('editor')}>Clean Canvas</button>
        </nav>
      </header>

      <aside className="control-panel">
        <div className="panel-heading"><span>Particle engine</span><span className="status-dot">● Live</span></div>
        <h1>{mode}</h1>
        <p className="muted">Interactive motion environment</p>
        <div className="mode-grid">
          {modes.map(item => <button key={item} className={mode === item ? 'mode active' : 'mode'} onClick={() => setMode(item)}>{item}</button>)}
        </div>
        <label>Particles <output>{particles.toLocaleString()}</output><input type="range" min="10000" max="500000" step="10000" value={particles} onChange={event => setParticles(Number(event.target.value))} /></label>
        <label>Speed <output>{speed.toFixed(1)}</output><input type="range" min="0" max="2" step="0.1" value={speed} onChange={event => setSpeed(Number(event.target.value))} /></label>
        <label>Intensity <output>{intensity.toFixed(1)}</output><input type="range" min="0" max="1" step="0.1" value={intensity} onChange={event => setIntensity(Number(event.target.value))} /></label>
        <div className="panel-actions"><button onClick={() => setPlaying(true)}>Reset</button><button onClick={() => setShowHelp(true)}>Keyboard help</button></div>
      </aside>

      <div className="editor-scene"><MotionScene /></div>
      <footer className="statusbar"><span>{playing ? 'Running' : 'Paused'}</span><span>{particles.toLocaleString()} particles</span><span>GPU renderer ready</span><span>Mode: {mode}</span></footer>
...
export default function App() {
  ...
  if (!opened) return <MotionScene onOpen={() => setOpened(true)} />;
  return <div className="app-root"><Editor panel={panel} setPanel={setPanel} />{panel !== 'editor' && <ResourcePanel type={panel} onClose={() => setPanel('editor')} />}</div>;
}
